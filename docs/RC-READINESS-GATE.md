...
| Ctrl+C                                              | `FAIL`                                  | RC-EVIDENCE-2 Interactive Batch 1: ping was terminated and prompt recovered, but native output was not real-time and the interrupted record showed failed/exit 0. | Ctrl+C stops the foreground workload, returns to a usable prompt, keeps the host/session alive and does not create a false successful record.             | YES — unless explicitly excluded |
