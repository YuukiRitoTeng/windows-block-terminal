...
| Python REPL                                         | `PASS`                                  | RC-EVIDENCE-2 Interactive Batch 1 records realtime entry/output, normal exit and a follow-up ordinary command using the preserved installer.                      | Realtime input/output, normal exit, return to the same outer session, no exact post-hoc output claim without a separate guarantee.                                                                                                                                                                                                                                                                                                                           | YES — unless explicitly excluded |
| Nested PowerShell                                   | `PASS`                                  | RC-EVIDENCE-2 Interactive Batch 1 records nested entry/exit, preserved outer PID/env/cwd and a follow-up ordinary command using the preserved installer.          | Nested shell remains one outer interactive workload, exits cleanly, and does not create a second authoritative hosted session.                                                                                                                                                                                                                                                                                                                               | YES — unless explicitly excluded |
...
| Ctrl+C                                              | `FAIL`                                  | RC-EVIDENCE-2 Interactive Batch 1: ping was terminated and prompt recovered, but native output was not real-time and the interrupted record showed failed/exit 0. | Native foreground output must be visible in real time; Ctrl+C must terminate the foreground workload; the prompt and host/session must remain usable; a subsequent ordinary command must succeed; the interrupted workload must not be recorded as successful; and status, exit/interruption state and signal metadata (if present) must be consistent. A `failed` record with authoritative exit 0 is not acceptable unless explicitly documented as valid. | YES — unless explicitly excluded |
