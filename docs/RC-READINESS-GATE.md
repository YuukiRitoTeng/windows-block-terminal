...
Repository/gate documentation baseline: current `main` documentation state;
the RC artifact product source SHA is fixed per candidate below.
...
- Clear Visual History removes visible history without restarting the shell,
  PTY or Runspace;
- the product direction is continuous-terminal first in each pane while retaining
  Wave tabs, split panes, workspaces and parallel terminal sessions;
...
- Installer size: `709442711` bytes
- Installer SHA-256: `0C388EDB436D5CCC9586938BDBF457F57C437A74A47D8F40076B1D55331B0344`
- Packaged hosted runtime SHA-256: `63E64F092A19550724E3ABC956B70D8C3A2B3EEEB57899639A4789048857A22C`
- Supported test configuration: Windows 11 x64, packaged Windows Block Terminal
  and packaged Hosted PowerShell runtime using the embedded
  `Microsoft.PowerShell.SDK` 7.4.12 engine.
...
## Current product/presentation direction

`docs/PRODUCT-DIRECTION.md` is the current product and presentation authority.
Windows Block Terminal remains a standalone application with a continuous
PowerShell-first terminal as the primary surface in every pane. Wave's tabs,
split panes, workspaces and parallel-terminal container capabilities remain
retained.

The target product no longer includes a traditional Command History inspector,
user-facing Command Cards, Web functionality or AI/Wave AI/BYOK onboarding.
These are product decisions awaiting bounded implementation/dependency audits;
they do not claim that current code or historical evidence has already removed
those surfaces. The Command Journal, `CommandRecord`, CVA, authenticated
structured output and trusted-output metadata remain reliability infrastructure.

The target local command action is one authoritative `Copy All` per command
region. Existing `Copy Command` and `Copy Output` controls are compatibility
surfaces, not separate target primary actions; they must continue to respect the
same authoritative binding and output guarantees. Clear remains global with
session preservation. Neither decision authorizes heuristic attribution; the
matrix below records the separate Copy All RC gate.
...
- embedded `Microsoft.PowerShell.SDK` 7.4.12 engine.

The `7.4.12` SDK reference in `tools/hostedpwsh/WbtHostedPowerShell.csproj` is
the authority for the embedded engine version. Historical evidence documents
record PowerShell 7.6.4 as the build/test host environment; they do not prove
that the packaged embedded engine was 7.6.4. External `pwsh` versions are
separate environment facts and are not substituted for the embedded runtime
version.
...
| `vim`                                                       | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Packaged manual PTY test                                     | YES                   | Portable Vim 9.2 editing/save/exit and terminal-mode restoration validated               |
| Alternate-screen                                            | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Packaged manual PTY test                                     | YES                   | Validated with the portable Vim workload; exact interactive output attribution excluded  |
| `fzf`                                                       | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Packaged manual PTY test                                     | YES                   | Portable fzf 0.74.3 selection/exit and parent-shell continuity validated                 |
| `ssh`                                                       | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Packaged manual PTY test                                     | YES                   | Localhost OpenSSH interactive smoke passed; remote output remains PTY-owned              |
| Resize / reflow during a workload                           | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Packaged manual resize test                                  | YES                   | Narrow/wide resize propagated during a live workload without visible corruption          |
| Hosted process / Runspace failure                           | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Deterministic production-seam and packaged recovery evidence | YES unless scoped out | Failure closes conservatively without a silent second session                            |
| Sidechannel disconnect                                      | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Deterministic authenticated disconnect evidence              | YES unless scoped out | Structured authority degrades; trusted output is not fabricated                          |
| Frontend reconnect                                          | `SUPPORTED_WITH_CONDITIONS` | `PASS`                                  | Packaged reconnect evidence                                  | YES unless scoped out | Durable history and live-terminal authority remain intact                                |
...
| `NOT YET TESTED`                        |     6 |
| `EVIDENCE EXISTS — REVALIDATION NEEDED` |    10 |
...
| --------------------------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------- |
| `vim`                                               | `PASS`         | M2A packaged Windows manual evidence records portable Vim 9.2 editing/save/exit and alternate-screen restoration with the parent shell usable.                                                                                                                       | Full-screen editing, input, exit, return to outer prompt, no false ordinary-command Card, no loss of terminal modes.                                                                                                                                                                                                                                                                                                                                         | YES — unless explicitly excluded |
| `fzf`                                               | `PASS`         | M2A packaged Windows manual evidence records portable fzf 0.74.3 interactive selection, selection of `charlie`, normal exit and parent-shell continuity.                                                                                                             | Interactive input and selection work, terminal modes restore, exit returns to the outer prompt.                                                                                                                                                                                                                                                                                                                                                              | YES — unless explicitly excluded |
| `ssh`                                               | `PASS`         | M2A packaged Windows manual evidence records a localhost OpenSSH interactive session using a temporary validation key, clean exit and restored authorization state.                                                                                                  | Local outer command remains one interactive record; remote input/output is not reclassified as local structured commands; exit restores the outer prompt.                                                                                                                                                                                                                                                                                                    | YES — unless explicitly excluded |
| Python REPL                                         | `PASS`         | RC-EVIDENCE-3 Targeted Revalidation records realtime entry/output, normal exit and a follow-up ordinary command using the preserved RC3 installer; RC2 is historical evidence.                                                                                       | Realtime input/output, normal exit, return to the same outer session, no exact post-hoc output claim without a separate guarantee.                                                                                                                                                                                                                                                                                                                           | YES — unless explicitly excluded |
| Nested PowerShell                                   | `PASS`         | RC-EVIDENCE-3 Targeted Revalidation records nested entry/exit, preserved outer PID/env/cwd and a follow-up ordinary command using the preserved RC3 installer; RC2 is historical evidence.                                                                           | Nested shell remains one outer interactive workload, exits cleanly, and does not create a second authoritative hosted session.                                                                                                                                                                                                                                                                                                                               | YES — unless explicitly excluded |
| Alternate-screen                                    | `PASS`         | M2A packaged Windows manual evidence records Vim alternate-screen entry/exit and correct restoration of the terminal surface.                                                                                                                                        | Alternate-screen enter/leave does not create ordinary Cards or corrupt the live xterm surface.                                                                                                                                                                                                                                                                                                                                                               | YES — unless explicitly excluded |
| Terminal resize / reflow                            | `PASS`         | M2A packaged Windows manual evidence records narrow/wide window changes during a live workload, propagated resize, no visible corruption/duplication and shell recovery.                                                                                             | Resize during an active workload preserves input, output, cursor state and return to the outer prompt.                                                                                                                                                                                                                                                                                                                                                       | YES — unless explicitly excluded |
| Ctrl+C                                              | `PASS`         | RC-EVIDENCE-3 Targeted Revalidation: ping replies were realtime, Ctrl+C terminated the foreground workload, prompt/session recovered, PID remained `32300`, follow-up succeeded, and the Card showed failed/exit 1/structured. RC2 remains historical FAIL evidence. | Native foreground output must be visible in real time; Ctrl+C must terminate the foreground workload; the prompt and host/session must remain usable; a subsequent ordinary command must succeed; the interrupted workload must not be recorded as successful; and status, exit/interruption state and signal metadata (if present) must be consistent. A `failed` record with authoritative exit 0 is not acceptable unless explicitly documented as valid. | YES — unless explicitly excluded |
| Interactive program exit → ordinary command resumes | `PASS`         | RC-EVIDENCE-3 Targeted Revalidation records Python REPL and nested PowerShell exit followed by ordinary commands with the same outer session.                                                                                                                        | The next ordinary command executes in the same supported session with preserved cwd/session state and no second shell/runspace.                                                                                                                                                                                                                                                                                                                              | YES — unless explicitly excluded |
...
permanently running record. The Slice 1 deterministic evidence and Slice 2
disposable packaged Windows evidence below are the current M2B authority.

| Scenario                           | Current Status                          | Existing Evidence                                                                                                                                                                                                                                                                                                                                                                       | Acceptance Criteria                                                                                                                                                     | RC Blocking If Failed?           |
| ---------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| Hosted PowerShell process failure  | `PASS`                                  | M2B Slice 1 deterministic production-seam and Windows hosted-child termination evidence show conservative interruption closure, stale identity rejection, no fabricated success and no silent replacement session.                                                                                                                                                                      | Active work is closed conservatively, no false success is written, the failure is observable, and a supported recovery path is available.                               | YES — unless explicitly excluded |
| Sidechannel disconnect             | `PASS`                                  | M2B Slice 1 evidence for PR #106 covers authenticated EOF/read failure, consumer degradation, conservative abort with `sidechannel_disconnected`, partial-output retention and rejection of late/stale events.                                                                                                                                                                          | Terminal remains safe, trusted structured output is not fabricated, active records become an explicit degraded/aborted state, and reconnection behavior is documented.  | YES — unless explicitly excluded |
| Shell / Runspace failure           | `PASS`                                  | M2B Slice 1 deterministic evidence covers interrupted closure, stale host/runspace/session rejection, no false success and no silent second authoritative session.                                                                                                                                                                                                                      | Runspace loss is visible, active records do not remain permanently running, and no second session is silently introduced.                                               | YES — unless explicitly excluded |
| Frontend reconnect                 | `PASS`                                  | M2B Slice 2 disposable VMware packaged evidence (TestDriver.ai Build run `33764290372`, source `d3e1a0bdc9dae5b34ce20afeda984791394b74e1`, installer SHA-256 `28576644c6498193e0cddd79a8d85fd8422ee255d46d4e2984e0b6a99d36fcbd`) records renderer recreation via `Ctrl+Shift+R`, unique A/B history with preserved order, no duplicate ingress and successful post-reconnect command R. | Reconnect does not resurrect stale records, lose durable records, or change the live terminal authority.                                                                | YES — unless explicitly excluded |
| Crash/recovery                     | `PASS`                                  | M2B Slice 2 disposable VMware packaged evidence (TestDriver.ai Build run `33764290372`, source `d3e1a0bdc9dae5b34ce20afeda984791394b74e1`, installer SHA-256 `28576644c6498193e0cddd79a8d85fd8422ee255d46d4e2984e0b6a99d36fcbd`) records full-process-tree crash/restart, conservative UNKNOWN recovery for incomplete work and normal post-recovery startup.                           | Finished durable records and safe output metadata restore; unfinished work is recovered conservatively; no data loss occurs.                                            | YES                              |
| Packaged app restart               | `PASS`                                  | RC-EVIDENCE-3 Build and prior installed-artifact evidence cover packaged hosted-resource identity/resolution; Recovery Batch 2 records normal close/restart, `WBT hosted PowerShell ready`, `persistence: available`, restoration of finished records without duplication, and a successful follow-up command.                                                                          | The declared RC artifact starts, wavesrv is ready, hosted resources resolve, and durable history remains available.                                                     | YES                              |
| Clear Visual History               | `PASS`                                  | RC-EVIDENCE-3 Recovery Batch 2 records product Clear Visual History clearing Cards and rendered scrollback while preserving PID, cwd, environment, function and session usability.                                                                                                                                                                                                      | Visible Cards and rendered scrollback clear; shell, PTY, host, Runspace, cwd and session state remain unchanged for the declared RC configuration.                      | YES                              |
...
| Backpressure / long-running output | `PASS`                                  | M2B Slice 1 deterministic bounded/degraded recovery evidence shows overflow remains conservative, incomplete records do not become falsely complete/trusted, subsequent commands remain usable and no heuristic attribution is used.                                                                                                                                                    | Product-owned accumulation remains bounded; degraded completeness/trust is surfaced conservatively; the command path remains recoverable after finite sustained output. | YES — unless explicitly excluded |

## 4. Release readiness

| Item                                       | Current Status                          | Existing Evidence                                                                                                                                                | Acceptance Criteria                                                                                                                                                                                                                                                                                | RC Blocking If Failed? |
| ------------------------------------------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| Windows x64 artifact                       | `PASS`                                  | RC-EVIDENCE-3 build and Recovery Batch 2 record the exact preserved NSIS artifact, installed application launch and packaged hosted runtime resolution.          | The declared Windows x64 RC artifact installs and launches in the supported configuration.                                                                                                                                                                                                         | YES                    |
...
| Remaining product branding cleanup         | `FAIL`                                  | The first identity pass is merged, but visible Wave-derived title, menu, updater, onboarding and asset surfaces remain.                                          | Default user-facing identity matches Windows Block Terminal's product direction, including removal/reframing of the traditional Command History inspector, user-facing Command Cards and unapproved Wave/Web/AI surfaces, while required legal attribution is preserved.                           | YES                    |
| Per-command Copy All                       | `EVIDENCE EXISTS — REVALIDATION NEEDED` | CVA causal binding and trusted structured-output/provenance paths are proven; the target local Copy All interaction is not yet validated on the final candidate. | A real CVA causal command-region → authoritative `CommandRecord` binding returns exactly that command plus only its corresponding trusted output. No row/text/prompt/timestamp/proximity/scrollback heuristic is allowed; packaged validation on the final candidate is required before RC `PASS`. | YES                    |
