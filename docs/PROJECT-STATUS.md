...
This document is the current authority for **project state and current stage**.

For current forward sequencing, priority, evidence discipline, smart-model checkpoints and manual Windows gates, read `RC-STRATEGIC-REBASELINE.md`.
...
If an older roadmap, Phase document, milestone label, README section, screenshot or status paragraph conflicts with these current authorities, the current authority documents win.
...
- M2A finite packaged interactive compatibility closure;
- M2B deterministic and packaged recovery closure, including authenticated sidechannel disconnect, frontend reconnect, crash/recovery, durable-history recovery and bounded backpressure.

The project is **not** yet a production-ready release.

M0, M1 and the finite M2A/M2B behavior/recovery scope are materially complete. The remaining Release Candidate Readiness work is product/release closure and exact-candidate revalidation, not a reopening of CVA, the supported-configuration contract, or the Conditional Architecture Freeze.
...
The architecture route remains valid. The product presentation and forward sequencing have been rebaselined.

Historical phase and milestone names should now be read as completed evidence checkpoints or historical mappings rather than the controlling development state:
...
- M0 CVA closure — complete;
- M1 supported-configuration closure — complete;
- M2A interactive compatibility closure — complete;
- M2B recovery closure — complete.

The older M3/M4/M5/M6 labels may still be useful when reading historical plans, but they no longer control forward sequencing. The controlling route is the five-stage RC closure sequence in `RC-STRATEGIC-REBASELINE.md`.
...
Architecture Review is **not triggered** by the continuous-terminal presentation reframe or the current RC strategic rebaseline.
...
- M2A finite packaged interactive scenarios;
- M2B finite recovery scenarios.
...
The finite M2A interactive matrix is complete on the packaged Windows target: Python REPL, nested PowerShell, external `pwsh` fallback, native Ctrl+C, Vim and alternate-screen, fzf, localhost SSH, and resize/reflow all have passing manual evidence. Interactive PTY workloads retain conservative semantics and do not gain an exact retrospective output-attribution promise.

M2B recovery evidence is **COMPLETE — PASS** for the finite Slice 1 and Slice 2 scope. Deterministic production-seam evidence covers hosted process/runspace failure, authenticated sidechannel disconnect and bounded backpressure. The disposable VMware packaged gate covers frontend reconnect, crash/recovery and durable-history recovery, including conservative representation of interrupted work and successful post-recovery append.

The conservative boundaries remain: interactive PTY workloads do not gain an exact retrospective output-attribution promise; crash recovery does not promise transparent continuation; xterm scrollback is not durable-history authority; and no second hosted PowerShell process or Runspace is introduced.

Historical M2A/M2B packaged evidence remains valid for its recorded artifact identities, but final release claims must be revalidated against the exact final signed RC candidate as required by `RC-STRATEGIC-REBASELINE.md` and `RC-READINESS-GATE.md`.

Background output attribution and exact interactive-copy semantics remain separate release/product questions.
...
- exact final-candidate provenance and packaged revalidation.
...
1. pre-freeze product / UX / identity closure;
2. performance / data / privacy / legal closure;
3. release / support chain closure;
4. final source freeze + exact signed-candidate revalidation;
5. RC decision / beta-support handoff.

The full sequencing, evidence discipline, smart-model checkpoints, manual Windows gates and STOP conditions are defined in `RC-STRATEGIC-REBASELINE.md`.

Before freezing the final release-candidate artifact, batch together artifact-affecting changes that still alter the final product experience or release behavior, including:
...
- repeat the Product Evidence Gate or already-closed M2 evidence without a new exact-candidate requirement.

## Evidence confidence

### Verified

Current architecture implementation, merged Product Evidence / Freeze / hardening / packaging / upgrade / first visual evidence, M0/M1 closure, finite M2A/M2B evidence, and candidate upstream rehearsal records.
