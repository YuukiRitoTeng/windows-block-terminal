...
app.setName("windows-block-terminal/electron");
...
app.setName(isDev ? "Windows Block Terminal (Dev)" : "Windows Block Terminal");
...
export function checkIfRunningUnderARM64Translation(fullConfig: FullConfigType) {
    if (!fullConfig.settings["app:dismissarchitecturewarning"] && app.runningUnderARM64Translation) {
        ...
        const dialogOpts: Electron.MessageBoxOptions = {
            ...
            title: "Windows Block Terminal has detected a performance issue",
            message: `Windows Block Terminal is running in ARM64 translation mode, which may impact performance.\n\nRecommendation: Download a native ARM64 version when one is available.`,
        };
