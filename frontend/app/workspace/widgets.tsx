...
function SettingsTooltipContent({ hasConfigErrors }: { hasConfigErrors: boolean }) {
    if (!hasConfigErrors) {
        return uiText("widgets.settingsHelp");
    }
    return (
        <div className="flex flex-col p-1">
            <div className="mb-1">{uiText("widgets.settingsHelp")}</div>
            <div className="flex items-center gap-1 mt-0.5 text-error">
                <i className="fa fa-solid fa-circle-exclamation"></i>
                <span>{uiText("widgets.configErrors")}</span>
            </div>
        </div>
    );
}
...
const SettingsFloatingWindow = memo(
    ({ isOpen, onClose, referenceElement, hasConfigErrors }: FloatingWindowPropsType) => {
        ...
        const menuItems = [
            ...
            {
                icon: "book-open",
                label: uiText("widgets.releaseNotes"),
                onClick: () => {
...
const Widgets = memo(() => {
    ...
    const handleWidgetsBarContextMenu = (e: React.MouseEvent) => {
        ...
        const menu: ContextMenuItem[] = [
            {
                label: uiText("widgets.editConfig"),
                click: () => {
                    fireAndForget(async () => {
                        const blockDef: BlockDef = {
                            meta: {
        ...
        <>
            ...
            >
                ...
                <div className="flex flex-col justify-center items-center w-full py-1.5 pr-0.5 text-lg">
                    ...
                    <div className="text-xxs mt-0.5 w-full px-0.5 text-center">{uiText("widgets.settings")}</div>
                </div>
