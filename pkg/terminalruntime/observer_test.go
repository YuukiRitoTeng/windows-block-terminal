...
func TestOutputObserverPreservesRawOrderAndSequences(t *testing.T) {
	...
	o := NewOutputObserver("session", NewDecoder(), func(chunk OutputChunk, events []IntegrationEvent) { results <- result{chunk, events} })
	...
	for i, want := range []string{"first", "second"} {
		select {
		case got := <-results:
			if got.chunk.Sequence != uint64(i+1) || string(got.chunk.Raw) != want || got.chunk.SessionEpoch != "session" {
